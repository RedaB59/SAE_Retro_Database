{{
  config(
    materialized  = 'incremental',
    unique_key    = 'id_parcelle_sk',
    post_hook     = [
      "{{ scd2_close_records(this, 'id_parcelle', 'hash_contenu', 'statut') }}",
      "CREATE INDEX IF NOT EXISTS idx_ods_parcelle_est_courant  ON {{ this }} (est_courant)",
      "CREATE INDEX IF NOT EXISTS idx_ods_parcelle_hash         ON {{ this }} (hash_contenu) WHERE est_courant = true",
      "CREATE INDEX IF NOT EXISTS idx_ods_parcelle_id           ON {{ this }} (id_parcelle, est_courant)",
      "ANALYZE {{ this }}"
    ]
  )
}}
with source as (
    select
        id_parcelle,
        ancien_id_parcelle,
        code_nature_culture,
        code_nature_culture_spe,
        surface_terrain,
        id_adresse,
        md5(concat_ws('|',
            id_parcelle,
            code_nature_culture,
            code_nature_culture_spe,
            surface_terrain::text
        ))                                  as hash_contenu
    from {{ ref('int_parcelles') }}
),

nouveaux as (
    select
        md5(concat_ws('|',
            s.id_parcelle,
            s.hash_contenu
        ))                                  as id_parcelle_sk,
        s.id_parcelle,
        s.ancien_id_parcelle,
        s.code_nature_culture,
        s.code_nature_culture_spe,
        s.surface_terrain,
        s.id_adresse,
        s.hash_contenu,
        current_timestamp                   as date_debut,
        '9999-12-31 00:00:00'::timestamp    as date_fin,
        true                                as est_courant,
        'ACTIF'                             as statut,
        current_timestamp                   as date_creation_ods,
        current_timestamp                   as date_maj_ods,
        'DSA'                               as source_systeme,
        1                                   as version
    from source s

    {% if is_incremental() %}
    where not exists (
        select 1 from {{ this }} t
        where t.hash_contenu = s.hash_contenu
          and t.est_courant = true
    )
    {% endif %}
)

select * from nouveaux